...
const GOOGLE_CONTAINER_IMAGE = 'google/cloud-sdk:400';
...
app.get('/run', function (req, res, next) {
  ...
  if (req.query.test == constants.LATENCY) {
    latencyRunningInterval = setInterval(function () {
      testingModule.get(req.query.test, req.query.testName, {});
      amountOfCallsCounter++;
      currentLogStatus = 'Running...<br>Counter: ' + amountOfCallsCounter;
    }, timeout);
  } else if (req.query.test == constants.FACTORS) {
    factorsRunningInterval = setInterval(function () {
      testingModule.get(req.query.test, req.query.testName, { n: req.query.n });
      amountOfCallsCounter++;
      currentLogStatus = 'Running...<br>Counter: ' + amountOfCallsCounter;
    }, timeout);
  } else if (req.query.test == constants.MATRIX) {
    matrixRunningInterval = setInterval(function () {
      testingModule.get(req.query.test, req.query.testName, { n: req.query.n });
      amountOfCallsCounter++;
      currentLogStatus = 'Running...<br>Counter: ' + amountOfCallsCounter;
    }, timeout);
  } else if (req.query.test == constants.FILESYSTEM) {
    filesystemRunningInterval = setInterval(function () {
      testingModule.get(req.query.test, req.query.testName, {
        n: req.query.n,
        size: req.query.size,
      });
    ...
    customRunningInterval = setInterval(function () {
      testingModule.get(req.query.test, req.query.testName, {});
      amountOfCallsCounter++;
      currentLogStatus = 'Running...<br>Counter: ' + amountOfCallsCounter;
    }, timeout);
...
app.get('/cleanupLogFile', function (req, res, next) {
  ...
  try {
    ...
    console.error(err);
    currentLogStatusEnd +=
      '<h4 style="color:red">Log File could not be cleaned up.</h4>';
    runningStatus = false;
  }
  res.send({ data: currentLogStatus, running: runningStatus });
});
  ...
  return new Promise((resolve, reject) => {
    exec(cmd, (error, stdout, stderr) => {
      if (error) {
        ...
        if (
          !(
            cmd.includes('az functionapp deployment source config-zip') &&
            error
              .toString()
              .includes("Operation failed with status: 'Bad Request'.")
          )
        ) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (params.dotnet == 'true') {
      let p = deployFunction(
        ...
        '/aws/src/dotnet/' + funcFirstUpperCase + '/',
        '.NET',
        '',
        '',
        params.ram,
...
async function deployAzure(params, func, funcFirstUpperCase, testName) {
  return new Promise(async (resolve, reject) => {
    ...
    currentLogStatusAzure +=
      '<h5>Microsoft Azure (Linux, parallel deployment)</h5>';
    ...
    if (params.go == 'true') {
      currentLogStatusAzure +=
        '<li><span style="color:orange">SKIP:</span> No Go runtime</li>';
    }
    if (params.dotnet == 'true') {
      let p = deployFunction(
        constants.AZURE,
...
async function deployAzureWindows(params, func, funcFirstUpperCase, testName) {
  return new Promise(async (resolve, reject) => {
    ...
    currentLogStatusAzureWindows +=
      '<h5>Microsoft Azure (Windows, parallel deployment)</h5>';
    ...
    if (params.python == 'true') {
      currentLogStatusAzureWindows +=
        '<li><span style="color:orange">SKIP:</span> No Python runtime</li>';
    }
    if (params.go == 'true') {
      currentLogStatusAzureWindows +=
        '<li><span style="color:orange">SKIP:</span> No Go runtime</li>';
...
async function deployGoogle(params, func, funcFirstUpperCase, testName) {
  return new Promise(async (resolve, reject) => {
    ...
    if (params.dotnet == 'true') {
      currentLogStatusGoogle +=
        '<li><span style="color:orange">SKIP:</span> No .NET runtime</li>';
    }
...
async function deployFunction(
  ...
  if (
    constants.PROVIDERS.includes(provider) &&
    ...
    if (provider == constants.AWS) {
      ...
      let dockerPrefixBothVolumes =
        'docker run --rm -v aws-secrets:/root/.aws -v serverless-data:' +
        dockerMountPoint +
        ' ' +
        AWS_CONTAINER_IMAGE +
        ' ';
      let dockerPrefixOnlyCLIVolume =
        'docker run --rm -v aws-secrets:/root/.aws ' +
        AWS_CONTAINER_IMAGE +
        ' ';
      ...
      if (language == constants.NODE) {
        ...
        srcPath =
          'fileb://' + dockerMountPoint + srcPath + functionName + '.zip';
        ...
        await execShellCommand(
          'docker run --rm -v serverless-data:' +
            dockerMountPoint +
            " golang:1.11-stretch /bin/sh -c 'cd " +
            dockerMountPoint +
            srcPath +
            "; go clean; go get github.com/aws/aws-lambda-go/lambda github.com/aws/aws-lambda-go/events; go build *.go'"
        ).catch((err) => {
          error = true;
          currentLogStatusAWS +=
        ...
        await execShellCommand(
          'docker run --rm -v serverless-data:' +
            dockerMountPoint +
            " mcr.microsoft.com/dotnet/core/sdk:2.2-alpine3.9 /bin/sh -c 'apk -uv add --no-cache zip; cd " +
            dockerMountPoint +
            srcPath +
            '; dotnet build; dotnet tool install -g Amazon.Lambda.Tools; dotnet lambda package -C Release -o ' +
            functionName +
            ".zip -f netcoreapp2.1'"
        ).catch((err) => {
          error = true;
      ...
      await execShellCommand(
        ...
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while creating lambda function. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      let lambdaarn = await execShellCommand(
        dockerPrefixOnlyCLIVolume +
          'aws lambda list-functions --query "Functions[?FunctionName==\\`' +
          functionName +
          '\\`].FunctionArn" --output text --region ' +
          config.aws.region
      ).catch((err) => {
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while getting the ARN of the lambda function. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      await execShellCommand(
        ...
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while creating REST API. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      let apiid = await execShellCommand(
        dockerPrefixOnlyCLIVolume +
        ...
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while getting the REST API ID. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      let parentresourceid = await execShellCommand(
        ...
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while getting the parent ID of the API. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      await execShellCommand(
        ...
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while creating resource on API. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      let resourceid = await execShellCommand(
        ...
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while getting the resource ID of the API. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      await execShellCommand(
        dockerPrefixOnlyCLIVolume +
          ...
          ' --resource-id ' +
          resourceid +
          ' --http-method ANY --authorization-type NONE --region ' +
          config.aws.region
      ).catch((err) => {
        error = true;
        currentLogStatusAWS +=
      ...
      await execShellCommand(
        dockerPrefixOnlyCLIVolume +
          ...
          ' --resource-id ' +
          resourceid +
          ' --http-method ANY --type AWS_PROXY --integration-http-method POST --uri arn:aws:apigateway:' +
          config.aws.region +
        ...
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while linking API to lambda. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      await execShellCommand(
        dockerPrefixOnlyCLIVolume +
          ...
          apiid +
          ' --stage-name test --region ' +
          config.aws.region
      ).catch((err) => {
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while creating deployment. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      await execShellCommand(
        ...
        error = true;
        currentLogStatusAWS +=
          '<li><span style="color:red">ERROR:</span> Error happened while giving lambda the API permission. Function ' +
          functionName +
          ' in language ' +
          languageName +
          ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      provider == constants.AZURE ||
      provider == constants.AZUREWINDOWS
    ) {
      let start = now();

      let dockerPrefixBothVolumes =
        'docker run --rm -v azure-secrets:/root/.azure -v serverless-data:' +
        dockerMountPoint +
        ' ' +
        AZURE_CONTAINER_IMAGE +
        ' ';
      let dockerPrefixOnlyCLIVolume =
        'docker run --rm -v azure-secrets:/root/.azure ' +
        AZURE_CONTAINER_IMAGE +
        ' ';
      ...
      await execShellCommand(
        ...
        if (provider == constants.AZURE) {
          currentLogStatusAzure +=
            '<li><span style="color:red">ERROR:</span> Error happened while creating resource group. Function ' +
            functionName +
            ' in language ' +
            languageName +
            ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      await execShellCommand(
        ...
        if (provider == constants.AZURE) {
          currentLogStatusAzure +=
            '<li><span style="color:red">ERROR:</span> Error happened while creating storage account. Function ' +
            functionName +
            ' in language ' +
            languageName +
            ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      await execShellCommand(
        ...
        if (provider == constants.AZURE) {
          currentLogStatusAzure +=
            '<li><span style="color:red">ERROR:</span> Error happened while creating function app. Function ' +
            functionName +
            ' in language ' +
            languageName +
            ' was <span style="font-weight: bold">NOT</span> deployed.</li>';
      ...
      if (error) {
        ...
        while (error) {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, errorCount * 1000)
          );
        }
      ...
      let dockerPrefixBothVolumes =
        'docker run --rm -v google-secrets:/root/.config/gcloud -v serverless-data:' +
        dockerMountPoint +
        ' ' +
      ...
      await execShellCommand(
        dockerPrefixBothVolumes +
          ...
          ' --memory=' +
          ram +
          config.google.memory_appendix +
          ' --timeout=' +
          timeout +
          config.google.timeout_appendix +
          ' --runtime=' +
          runtime +
          ' --trigger-http --source=' +
          dockerMountPoint +
...
async function cleanupAWS() {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < config.aws.region_options.length; i++) {
      let p1 = execShellCommand(
        'docker run --rm -v aws-secrets:/root/.aws ' +
          AWS_CONTAINER_IMAGE +
          ' aws lambda list-functions --region ' +
          config.aws.region_options[i]
      )
        .then((stdout) => {
          let awslambda = JSON.parse(stdout);
          for (let j = 0; j < awslambda.Functions.length; j++) {
            awsFunctions.push({
              function: awslambda.Functions[j].FunctionName,
              region: config.aws.region_options[i],
            });
          }
        })
        .catch((err) => {
          currentLogStatusAWS +=
            '<li><span style="color:red">ERROR:</span> Could not load existing AWS lambda functions</li>';
          error = true;
        });
      promises.push(p1);

      let p2 = execShellCommand(
        'docker run --rm -v aws-secrets:/root/.aws ' +
          AWS_CONTAINER_IMAGE +
        ...
        .catch((err) => {
          currentLogStatusAWS +=
            '<li><span style="color:red">ERROR:</span> Could not load existing AWS APIs</li>';
          error = true;
        });
      promises.push(p2);
    }
    ...
    if (error) {
      ...
      currentLogStatusAWS +=
        ...
        millisToMinutesAndSeconds((now() - start).toFixed(3));
    }

    if (awsFunctions.length == 0 && awsGateways.length == 0) {
      currentLogStatusAWS +=
        '<li><span style="color:orange">SKIP:</span> Nothing to clean up.</li>';
    }
    ...
    for (let i = 0; i < awsFunctions.length; i++) {
      let start = now();
      let p = execShellCommand(
        'docker run --rm -v aws-secrets:/root/.aws ' +
          AWS_CONTAINER_IMAGE +
          ' aws lambda delete-function --function-name ' +
          awsFunctions[i].function +
          ' --region ' +
          awsFunctions[i].region
      )
        .then((stdout) => {
          let end = now();
          let time = millisToMinutesAndSeconds((end - start).toFixed(3));
          currentLogStatusAWS +=
            '<li><span style="color:green">INFO:</span> Function "' +
            awsFunctions[i].function +
            ' in ' +
            awsFunctions[i].region +
            '" deleted ' +
        ...
        .catch((err) => {
          currentLogStatusAWS +=
            '<li><span style="color:red">ERROR:</span> Function "' +
            awsFunctions[i].function +
            ' in ' +
            awsFunctions[i].region +
    ...
    for (let i = 0; i < awsGateways.length; i++) {
      ...
      await execShellCommand(
        'docker run --rm -v aws-secrets:/root/.aws ' +
          ...
          awsGateways[i].api +
          ' --region ' +
          awsGateways[i].region
      )
        .then((stdout) => {
          let end = now();
          let time = millisToMinutesAndSeconds((end - start).toFixed(3));
          currentLogStatusAWS +=
            '<li><span style="color:green">INFO:</span> API with ID "' +
            awsGateways[i].api +
            ' in ' +
            awsGateways[i].region +
            '" deleted ' +
        ...
        .catch((err) => {
          currentLogStatusAWS +=
            '<li><span style="color:red">ERROR:</span> API with ID "' +
            awsGateways[i].api +
            ' in ' +
            awsGateways[i].region +
            '" could not be deleted</li>';
    ...
    await execShellCommand(
      ...
      .then((stdout) => {
        ...
        for (let i = 0; i < azureresourcegroups.length; i++) {
          if (
            azureresourcegroups[i].name.includes('latency') ||
            azureresourcegroups[i].name.includes('factors') ||
            azureresourcegroups[i].name.includes('matrix') ||
            azureresourcegroups[i].name.includes('filesystem') ||
            azureresourcegroups[i].name.includes('custom')
          ) {
      ...
      .catch((err) => {
        currentLogStatusAzure +=
          '<li><span style="color:red">ERROR:</span> Could not load existing Azure resource groups</li>';
        error = true;
      });
    ...
    for (let i = 0; i < azureResourceGroups.length; i++) {
      ...
      let p = execShellCommand(
        ...
        .then((stdout) => {
          ...
          let time = millisToMinutesAndSeconds((end - start).toFixed(3));
          currentLogStatusAzure +=
            '<li><span style="color:green">INFO:</span> Function App in RG "' +
            azureResourceGroups[i] +
            '" deleted ' +
            time +
            '</li>';
        ...
        .catch((err) => {
          currentLogStatusAzure +=
            '<li><span style="color:red">ERROR:</span> Function App in RG "' +
            azureResourceGroups[i] +
            '" could not be deleted</li>';
        });
      promises.push(p);
...
async function cleanupGoogle() {
  return new Promise(async (resolve, reject) => {
    ...
    await execShellCommand(
      'docker run --rm -v google-secrets:/root/.config/gcloud ' +
        GOOGLE_CONTAINER_IMAGE +
      ...
      .catch((err) => {
        currentLogStatusGoogle +=
          '<li><span style="color:red">ERROR:</span> Could not load existing Google Cloud functions</li>';
        error = true;
      });
    ...
    for (let i = 0; i < googleFunctions.length; i++) {
      ...
      let p = execShellCommand(
        'docker run --rm -v google-secrets:/root/.config/gcloud ' +
          ...
          googleFunctions[i].function +
          ' --region ' +
          googleFunctions[i].region +
          ' --quiet'
      )
        .then((stdout) => {
          let end = now();
