...
function pushURL(test, provider, language, url, memory, region) {
  if (
    ...
    fs.writeFile(
      './urls/' + constants.TESTS[index] + '_urls.json',
      JSON.stringify(allUrls[test]),
      function (err) {
        if (err) {
          console.error(err);
        }
...
function resetURLs() {
  for (let i = 0; i < Object.keys(allUrls).length; i++) {
    fs.writeFile(
      './urls/' + constants.TESTS[i] + '_urls.json',
      '[]',
      function (err) {
        if (err) {
          console.error(err);
...
function insertIntoDB(
  ...
  if (
    test == constants.FACTORS ||
    test == constants.MATRIX ||
    test == constants.FILESYSTEM ||
    test == constants.CUSTOM
  ) {
