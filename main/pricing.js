...
function calcPrice(calls, execTime, kb, memory, provider) {
  let invocations_net =
    calls - prices[provider].freeCalls > 0
      ? calls - prices[provider].freeCalls
      : 0;
  let gb_seconds_net =
    ((execTime / 1000) * calls * memory) / 1024 - prices[provider].freeGBSec > 0
      ? ((execTime / 1000) * calls * memory) / 1024 - prices[provider].freeGBSec
      : 0;
  let ghz_seconds_net =
    (execTime / 1000) *
      calls *
      functionSizes[functionSizes.findIndex((x) => x.memory == memory)].cpu -
      prices[provider].freeGHzSec >
    0
      ? (execTime / 1000) *
          calls *
          functionSizes[functionSizes.findIndex((x) => x.memory == memory)]
            .cpu -
        prices[provider].freeGHzSec
      : 0;
  let networking_net =
    (calls * kb) / 1024 / 1024 - prices[provider].freeGBNetwork > 0
      ? (calls * kb) / 1024 / 1024 - prices[provider].freeGBNetwork
      : 0;

  let ghz_seconds_gross =
    (execTime / 1000) *
    calls *
    functionSizes[functionSizes.findIndex((x) => x.memory == memory)].cpu;
  ...
  var table = {
    ...
    total_price:
      invocations_net * prices[provider].perCall +
...
async function calcAllPricesFromTest(calls, testName, test, runtime) {
  ...
  var azureWindowsTable = calcPrice(
    calls,
    execTimeAzureWindows,
    kb,
    memory,
    'azure'
  );
